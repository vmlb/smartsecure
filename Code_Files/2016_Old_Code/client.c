...
THREAD_RETURN WOLFSSL_THREAD client_test(void* args)
{
    ...
    char msg[32] = "Hello from IoT user!";   /* GET may make bigger */
    char resumeMsg[32] = "Resuming Device Connection!";
#else
    char msg[32] = "Hello from IoT user!\n";
    char resumeMsg[32] = "Resuming Device Connection!\n";
#endif
...
/* allow some time for exporting the session */
    ...
    char newMsg[80];
    scanf("%79[^\n]", newMsg);
    int newMsgSz = (int)XSTRLEN(newMsg);
    if (wolfSSL_write(ssl, newMsg, newMsgSz) != newMsgSz) {
        wolfSSL_free(ssl);
        wolfSSL_CTX_free(ctx);
